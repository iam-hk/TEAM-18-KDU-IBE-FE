...
const PropertyConfigSlice = createSlice({
  ...
  initialState,
  reducers: {
    updateGuestCounts: (state,action: PayloadAction<{ index: number; increment: boolean }>) => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getPropertyConfig.fulfilled, (state, action:PayloadAction<IPropertyConfigSlice>) => {
        state.state = "fulfilled";
        state.guests=action.payload.guests;
        state.guestCounts=state.guests.map((item) => (item.type === "Adults" ? 1 : 0));
        ...
        const adultIndex = state.guests.findIndex((guest) => guest.type === "Adults");
