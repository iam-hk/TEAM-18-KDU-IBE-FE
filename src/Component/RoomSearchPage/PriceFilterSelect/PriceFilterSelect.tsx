...
import { changeSelectedSortingParam, changeSortingTechnique } from '../../../redux/FilterRoomSlice';
...
const PriceFilterSelect = (props:IDropDownSort) => {
  ...
  const selectedSortingTechnique = useSelector((state:RootState)=>state.filterRoom.selectedSortingOrder);
  ...
  const handleOptionClick = (variable: string,sort: boolean) => {
    ...
    if(variable !="Select")
    reduxDispatch(changeSortingTechnique(sort));
    setIsOpen(false);
  };
    ...
    <div className="custom-select">
      ...
      {isOpen && (
        <div className="options">
          <div className='option' onClick={()=>handleOptionClick('Select',false)}>Select</div>
          {
              props.sorts.map((individualOption) => {
                return (
                  <div
                    className='option'
                    onClick={() => handleOptionClick(individualOption.variable, individualOption.order)}
                    data-variable={individualOption.variable}
                    data-sort={individualOption.order}
                    key={individualOption.sortDisplayName} // Adding a unique key for each div
                  >
